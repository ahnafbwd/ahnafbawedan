...
class ProjectManager {
    ...
    async loadProjects() {
        try {
            ...
            // Create sample project data if loading fails
            this.projects = [{
                id: 1,
                title: "Sample Project",
                description: "This is a sample project shown while loading actual project data.",
    ...
    renderProjectCard(project) {
        ...
        const techBadges = project.technologies.map((tech, index) => 
            `<span class="inline-block ${this.getTechBadgeColor(index)} px-2 py-1 rounded-md text-xs font-medium">${tech}</span>`
        ).join('');

        return `
            <div class="project-item card p-4 animate-on-scroll cursor-pointer hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 group" 
                 data-category="${project.category}" 
                 onclick="projectsManager.handleProjectClick(${project.id}, this)"
                ...
                <div class="relative overflow-hidden rounded-lg mb-4 h-48">
                    ...
                    <div class="absolute top-2 right-2 bg-white/20 backdrop-blur-sm rounded-full p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center text-primary group-hover:text-secondary transition-colors">
                        <span class="text-sm mr-2 font-medium">View Details</span>
    ...
    updateProjectItems() {
        ...
        projectItems.forEach((item, index) => {
            ...
            setTimeout(() => {
                ...
                // Add visible class for scroll animation
    ...
    handleProjectClick(projectId, element) {
        ...
        loadingOverlay.className = 'absolute inset-0 bg-white/80 flex items-center justify-center z-10 rounded-lg';
        ...
        // Show project details after short delay
        setTimeout(() => {
    ...
    showProjectDetails(projectId) {
        console.log('Showing project details for ID:', projectId, 'Type:', typeof projectId);
        console.log('Available projects:', this.projects.map(p => ({ id: p.id, title: p.title, idType: typeof p.id })));
        
        // Convert projectId to number to handle type mismatch
        const numericProjectId = parseInt(projectId);
        const project = this.projects.find(p => p.id == projectId || p.id == numericProjectId);
                    ...
                    <div class="relative bg-gradient-to-r from-primary to-secondary text-white">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-r from-primary/90 to-secondary/90 flex items-center">
                            <div class="container mx-auto px-6 flex justify-between items-center">
                                ...
                                <button onclick="projectsManager.closeProjectModal()" class="bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-full w-10 h-10 flex items-center justify-center transition group">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div class="lg:col-span-2 space-y-6">
                                    ...
                                    <div class="bg-white border border-gray-200 rounded-lg p-4">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div class="aspect-video rounded-lg overflow-hidden cursor-pointer hover:shadow-md transition-shadow group">
                                                <img src="${project.image}" alt="${project.title} - Main Screenshot" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                                            </div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-2 text-center">Click images to view in full size</p>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="bg-gradient-to-br from-primary to-secondary text-white rounded-lg p-4">
                                        ...
                                        <a href="#contact" onclick="projectsManager.closeProjectModal()" class="block w-full text-center bg-white text-primary font-bold py-2 rounded-lg hover:bg-gray-100 transition text-sm">
                                            Get In Touch
                                        </a>
                                    </div>
                                </div>
            ...
            if (container) {
                container.innerHTML = `
                    <div class="col-span-full text-center py-8">
                        ...
                        <h3 class="text-lg font-bold mb-2">Unable to Load Projects</h3>
                        <p class="text-gray-600 mb-4">There was an error loading the project data. Please refresh the page to try again.</p>
                        <button onclick="window.location.reload()" class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-blue-600 transition">
                            Refresh Page
                        </button>
                    </div>
                `;
...
// Initialize project manager when the script loads
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        // Will be initialized by the main script
    });
} else {
    // DOM is already loaded, but wait for main script initialization
    console.log('📋 ProjectManager class ready for initialization');
}
